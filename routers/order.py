...
from emails.order_coming_today import OrderComingTodayEmail
...
def send_order_coming_today_email_best_effort(email: str, first_name: str, estimated_delivery: str,
    ...
    order_coming_today_email: OrderComingTodayEmail = OrderComingTodayEmail(
        url_base=url_base,
        first_name=first_name,
    ...
    # send email best effort
    try:
        send_email(jinjaEnv, order_coming_today_email, smtp_server, settings.email, settings.email_app_password)
    except Exception:
        logger.exception("Failed to send email.")
...
@router.post("/order_coming_today")
async def order_coming_today(user_facing_order_id: str, db: Session = Depends(get_db)):
    order: models.orders.Order = db.query(models.orders.Order).filter(
    ...
    estimated_delivery_str: str = datetime.now().strftime("%b %-d")

    order_dict = to_order_dict(order, db, customer_email=customer.email)

    send_order_coming_today_email_best_effort(customer.email, order.recipient_first_name, estimated_delivery_str,
                                              order_dict)
...
async def order_enroute(user_facing_order_id: str, estimated_delivery: datetime, db: Session = Depends(get_db)):
    ...
    send_order_enroute_email_best_effort(customer.email, order.recipient_first_name, estimated_delivery_str,
                                         order_dict)
